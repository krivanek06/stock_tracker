...
export class FreeCashflowFormulaService {
	private formula$: BehaviorSubject<StFreeCashFlowFormula | null> = new BehaviorSubject<StFreeCashFlowFormula | null>(null);

	constructor(private symbolStorageService: SymbolStorageService) {
		this.watchActiveSymbolChange();
	}

	get freeCashFlowFormula(): StFreeCashFlowFormula | null {
		return this.formula$.value;
	}

	getFreeCashFlowFormula(): Observable<StFreeCashFlowFormula | null> {
		return this.formula$.asObservable();
	}

	calculateFormulaResult(applyNetBorrowings: boolean) {
		if (!this.freeCashFlowFormula) {
			return;
		}
