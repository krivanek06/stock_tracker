...
export class NotificationProgressDirective {
  ...
  */
	@Input() notificationProgressSeconds = 1;

	private state$: BehaviorSubject<'start' | 'cancel'> = new BehaviorSubject<'start' | 'cancel'>('cancel');

	private cancel$: Observable<string>;

	constructor(private _snackBar: MatSnackBar, private notificationProgressService: NotificationProgressService, private __matDialog: MatDialog) {
	...
	private async openDialog(message: string): Promise<boolean> {
		const dialogRef = this.__matDialog.open(ConfirmDialogComponent, {
			data: {
				dialogTitle: message,
