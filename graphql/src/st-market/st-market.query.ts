...
export const querySTMarketHistoryOverview = async (): Promise<api.STMarketHistoryOverview> => {
	try {
		...
		const dataDoc = await admin.firestore().collection(api.ST_SHARED_ENUM.ST_COLLECTION).doc(api.ST_SHARED_ENUM.MARKET_HISTORY_OVERVIEW).get();
...
export const queryMarketDailyOverview = async (): Promise<api.STMarketDailyOverview> => {
	try {
		const dailyInfoDoc = await admin.firestore().collection(api.ST_SHARED_ENUM.ST_COLLECTION).doc(api.ST_SHARED_ENUM.MARKET_DAILY_OVERVIEW).get();
...
export const queryStMarketData = async (key: string): Promise<api.STMarketChartDataResultCombined> => {
	try {
		...
		let firebaseData = dataDoc.data() as api.STMarketChartDataResultAPI;

		// does not exists or too old data
		if (!firebaseData || !firebaseData.lastUpdate || moment(new Date()).diff(moment(firebaseData.lastUpdate), 'days') > 7) {
			console.log(`quandl update: ${key}`);
